...
uint8_t SBGC_cmd_realtime_data0_unpack(SBGC_cmd_realtime_data0_t &p, SerialCommand &cmd) {
		...
		for(uint8_t i=0; i<3; i++) {
			p.sensor_data[i].acc_data = cmd.readWord();
			p.sensor_data[i].gyro_data = cmd.readWord();
            cmd.skipBytes(2); // reserved
		}
		//p.serial_error_cnt = cmd.readWord();
		//p.system_error = cmd.readWord();
		//cmd.skipBytes(4); // reserved
        p.debug = cmd.readWord();
		cmd.readWordArr(p.rc_raw_data, 3);
        p.rc_cmd = cmd.readWord();
        p.ext_fc_roll = cmd.readWord();
        p.ext_fc_pitch = cmd.readWord();
        cmd.readWordArr(p.camera_angle, 3);
        cmd.readWordArr(p.rc_angle, 3);
		cmd.readWordArr(p.frame_imu_angle, 3);
		p.cycle_time_us = cmd.readWord();
		p.i2c_error_count = cmd.readWord();
        p.error_code = cmd.readByte();
        p.error_code_ext = cmd.readWord();
		p.battery_voltage = cmd.readWord();
		p.state_flags1 = cmd.readByte();
		p.cur_profile = cmd.readByte();        
		p.cur_imu = cmd.readByte();
...
uint8_t SBGC_cmd_getangles_data_unpack(SBGC_cmd_getAngles_data_t &p, SerialCommand &cmd) {
	...
	#else
		for(uint8_t i=0; i<3; i++) {
			p.angleData[i].angle = cmd.readWord();
			p.angleData[i].rc_angle = cmd.readWord();
            p.angleData[i].rc_speed = cmd.readWord();
        }
