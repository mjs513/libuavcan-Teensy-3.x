...
//Added by MJS 8/29/18
typedef struct {
	struct {
		int16_t acc_data;
		int16_t gyro_data;
        int16_t reserved;
	} sensor_data[3];  // ACC and Gyro raw sensor data
	int16_t debug; // debug variables
	int16_t rc_raw_data[3]; // RC signal in 1000..2000 range for ROLL, PITCH, YAW
    int16_t rc_cmd; //RC command channel value (PWM or normalized analog)
    int16_t ext_fc_roll;  //External FC PWM values. May be zero if their
    int16_t ext_fc_pitch; //inputs are mapped to RC control or command
	int16_t camera_angle[3]; // ROLL, PITCH, YAW Euler angles of a camera, 16384/360 degrees (-32768-32768) in 14 bit resolution   
	int16_t rc_angle[3]; // ROLL, PITCH, YAW Euler angles in 14-bit resolution
	int16_t frame_imu_angle[3]; // ROLL, PITCH, YAW Euler angles of a frame, if known, detected by second IMU
	uint16_t cycle_time_us; // cycle time in us. Normally should be 800us
	uint16_t i2c_error_count; // I2C errors counter
    uint8_t error_code;
	...
	uint8_t cur_profile; // active profile number starting from 0
	uint8_t cur_imu; // actually selecteted IMU for monitoring. 1: main IMU, 2: frame IMU
} SBGC_cmd_realtime_data0_t;

uint8_t SBGC_cmd_realtime_data0_unpack(SBGC_cmd_realtime_data0_t &p, SerialCommand &cmd);
...
} SBGC_cmd_getAngles_data_t;

uint8_t SBGC_cmd_getangles_data_unpack(SBGC_cmd_getAngles_data_t &p, SerialCommand &cmd);
